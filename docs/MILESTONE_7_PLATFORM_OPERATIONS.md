...
M7 remains **in progress**, not complete. The operator has now supplied `hotelmate.ir`, WebRamz DNS, and the ACME contact, and the hardened VPS is validated as trusted HTTPS staging. [ADR-0007](adr/0007-platform-operations-decisions.md) is still unapproved for production and no registry authorization, protected production environment, off-host restic destination, secrets manager, recovery objectives, paging receiver, or replacement host was supplied. The VPS cannot truthfully be treated as production or prove protected production promotion, encrypted off-host durability, approved recovery objectives, routed-alert evidence, or replacement-host recovery.

### External staging verification evidence

The sanitized record is [M7 external staging validation — 2026-08-23, updated 2026-08-25](evidence/M7_STAGING_VALIDATION_20260823.md). The supplied host was bootstrapped through Ansible, rebooted, and converged to a zero-change repeat run. Key-only non-root SSH, default-deny UFW, private PostgreSQL, bounded Docker logs, unattended updates, fail2ban, time synchronization, a 2 GiB swap file, and checksum-verified CLI/Cosign installation were verified.

All seven migrations, public/authenticated smoke, and the native stateful acceptance suite passed remotely. A staging recovery set containing PostgreSQL plus the private-upload scope was created and independently verified with matching hashes and 162 PostgreSQL catalog entries. The exercise also exposed and fixed the API/backup-operator upload permission boundary; the shared group contract is now enforced in the application, Compose, restore path, tests, and Ansible. On 2026-08-25, the root and `www` names converged on both authoritative servers, Let's Encrypt issued a trusted ECDSA certificate, HTTP began redirecting to HTTPS, HSTS/security headers were verified, Certbot was converted to the mounted webroot, and a dry-run renewal plus post-renewal Nginx reload passed.

This staging deployment uses trusted HTTPS, controller-loaded immutable Linux/amd64 images, and a local-only recovery set. Private GHCR access remains unavailable, so registry/signature gates were not bypassed; scheduled operations and the resource-heavy observability profile remain disabled until off-host storage and an approved alert receiver exist.
...
[CI #30](https://github.com/TablazOrg/HotelMate/actions/runs/32834547422) passed all five gates for the codified certificate-renewal hook. The release job in [Release and deploy #11](https://github.com/TablazOrg/HotelMate/actions/runs/32834669747) again published and verified the artifacts; its staging job reached the same intentionally unresolved protected-environment SSH gate, and production was skipped.
...
| CI and supply chain | Pinned CI/release workflows, tests, scans, SBOMs, provenance, cosign verification, immutable release manifest | Implemented and remotely proven by CI #26/release #8 and renewal-automation CI #30/release #11; branch and production-environment enforcement pending |
...
| Host baseline | Versioned Ansible, firewall, key-only user, updates, swap, log rotation, certbot timer/deploy hook, protected directories, checksum verification | Applied to an external Ubuntu host, rebooted, converged with `changed=0`, and passed simulated certificate renewal/reload; replacement-host rebuild still pending |
