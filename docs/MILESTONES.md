...
## M7 — Platform operations, CLI, CI/CD, backup, and infrastructure (complete with two approved exceptions)
...
Delivered in the repository with the unified Go operations CLI and JSON/exit-code contract; a native Go stateful acceptance suite; mandatory confirmation and signal-safe environment locks; digest-pinned production Compose; pinned, scan-gated, SBOM/provenance/signing release automation; staging-to-protected-production promotion; strict PostgreSQL/private-upload recovery manifests; scheduled retention; deploy evidence and application rollback; Prometheus/Grafana/Alertmanager/Loki/Alloy monitoring with PostgreSQL query telemetry; and an Ansible host-hardening baseline.

On 2026-08-25 the protected GitHub workflow built, scanned, SBOMed, attested, signed, and verified release `0.7.0-884b7e7d7e22`, deployed its immutable digests to staging, and promoted those same digests through the production approval gate. Production at <https://hotelmate.ir> passed public and authenticated smoke, eight migrations, coordinated backup creation and verification, scheduled purge jobs, monitoring readiness with all six Prometheus targets healthy, firewall and SSH-policy checks, TLS renewal checks, and repeat Ansible convergence with no drift. [ADR-0007](adr/0007-platform-operations-decisions.md) records the accepted single-VPS topology, objectives, access policy, and operational ownership; the [production evidence record](evidence/M7_PRODUCTION_VALIDATION_20260825.md) contains the sanitized release and runtime evidence.

The owner explicitly accepted two post-M7 risks: recovery sets are currently local to the VPS (`offHost: false`) until restricted S3-compatible storage is supplied, and Alertmanager currently uses a local no-op receiver until an approved webhook or SMTP app password is supplied. These exceptions do not claim off-host durability or external alert delivery; they remain tracked operational debt. The detailed implementation, acceptance gates, and exception boundaries are maintained in [M7 platform operations and infrastructure](MILESTONE_7_PLATFORM_OPERATIONS.md).

## M8 — Online check-in 2.0 and arrival readiness (complete)
...
Delivered with purpose-bound signed invitations and QR recovery, hashed/revocable capabilities, fail-closed tenant controls, a resumable three-step guest journey, partial autosave, companions, multiple private evidence files, unsafe-content rejection, a provider-neutral identity-verification boundary, versioned terms and accessible drawn/text signatures, idempotent submit/resubmit, correction and cancellation, automatic expiry, room-readiness and physical-arrival closure, and the pre-arrival entitlement boundary. The staff workspace adds readiness scoring, risk/outstanding state, ETA and ownership filters, reminders, reasoned review, evidence access, room-ready actions, and tenant funnel/handling analytics. Migration `2026082308_arrival_readiness`, the complete OpenAPI contract, retention integration, native stateful acceptance coverage, and the responsive Persian RTL React experience are included.

Release `0.8.0-local` is deployed through the guarded Compose release path with migration, smoke, and full in-container acceptance evidence. See [M8 implementation and operation](MILESTONE_8_ONLINE_CHECKIN.md) and [local validation evidence](evidence/M8_LOCAL_VALIDATION_20260824.md). Moderated participant research, product-owner pilot targets, and public production rollout remain explicit general-availability gates rather than unverified engineering claims.
...
The benchmark, current-state UX audit, detailed scope, sequencing, product measures, and acceptance gates for M8–M12 are defined in [Product improvement milestones](PRODUCT_IMPROVEMENT_MILESTONES.md). M8 is engineering-complete; M9–M12 remain planned.
