...
export function VisitorExperience() {
  ...
  if (!selectedSlug || (!content && !loading)) return <main className="auth-shell"><section className="auth-mobile visitor-lookup"><div className="auth-brand"><img src="/hotelmate-logo.svg" alt="" /><strong>HotelMate</strong></div><div className="auth-intro"><span><i className="ri-hotel-line" /></span><h1>مشاهده امکانات هتل</h1><p>شناسه هتل را وارد کنید تا امکانات، پیشنهادها و منوی رستوران را بدون ورود ببینید.</p></div><form className="auth-form" onSubmit={findHotel}><label>شناسه هتل<input dir="ltr" value={slug} onChange={(event) => setSlug(event.target.value)} placeholder="example-hotel" required /></label>{error && <div className="inline-alert error">{error}</div>}<button className="primary-button">مشاهده هتل</button></form><nav className="auth-links"><a href="/">مهمان هتل هستید؟ وارد شوید <i className="ri-arrow-left-line" /></a></nav></section></main>
